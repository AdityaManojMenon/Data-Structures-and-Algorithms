...
class Graph:
    ...
    def bfs(self, begin_id: str, end_id: str) -> Tuple[List[str], float]:
        """
        Perform a Breadth-First Search (BFS) from a starting vertex to an ending vertex in the graph.

        This method finds the shortest path in terms of the number of edges traversed, taking into account the weights of edges.

        Parameters:
            begin_id (str): The unique identifier for the starting vertex.
            end_id (str): The unique identifier for the ending vertex.

        Returns:
            Tuple[List[str], float]: A tuple containing a list of vertex identifiers representing the path from the start vertex to the end vertex, and the total weight of the path. Returns ([], 0) if no path is found.
        """
        ...
        while not queue.empty():
            ...
            for neighbor_id, weight in current_vertex.adj.items():
                if neighbor_id not in predecessors: 
                    ...
                    distances[neighbor_id] = distances[current_id] + weight
                    if neighbor_id == end_id:
                        return self.build_path(predecessors, begin_id, end_id)
    ...
    def a_star(self, start_id: str, target_id: str,
        ...
        """
        Perform an A* search algorithm to find the shortest path from a start vertex to a target vertex based on a heuristic metric.

        A* search combines features of uniform-cost search and pure heuristic search to efficiently compute optimal solutions.

        Parameters:
            start_id (str): The unique identifier for the starting vertex.
            target_id (str): The unique identifier for the target vertex.
            metric (Callable[[Vertex, Vertex], float]): A function to estimate the cost from the current vertex to the target vertex. Commonly used metrics include Euclidean and Taxicab distances.

        Returns:
            Tuple[List[str], float]: A tuple containing a list of vertex identifiers that make up the shortest path from the start vertex to the target vertex and the total weight of this path. Returns ([], 0) if no path exists.
        """
        if start_id not in self.vertices or target_id not in self.vertices:
            return [], 0

        open_set = PriorityQueue()
        open_set.push(0 + metric(self.vertices[start_id], self.vertices[target_id]), self.vertices[start_id])
        
        came_from = {}
        g_score = {vertex: float('inf') for vertex in self.vertices}
        g_score[start_id] = 0
        f_score = {vertex: float('inf') for vertex in self.vertices}
        f_score[start_id] = metric(self.vertices[start_id], self.vertices[target_id])

        while not open_set.empty():
            _, current = open_set.pop()

            if current.id == target_id:
                return self.build_path(came_from, start_id, target_id)
            ...
            for neighbor_id, weight in current.adj.items():
                ...
                if tentative_g_score < g_score[neighbor_id]:
                    ...
                    f_score[neighbor_id] = g_score[neighbor_id] + metric(self.vertices[neighbor_id], self.vertices[target_id])
                    if neighbor_id not in open_set.locator:
                        open_set.push(f_score[neighbor_id], self.vertices[neighbor_id])
                    else:
                        open_set.update(f_score[neighbor_id], self.vertices[neighbor_id])

        return [], 0

    def tollways_algorithm(self, start_id: str, target_id: str, coupons: int) -> Tuple[float, float] | Tuple[None, None]:
        """
        Calculates the cheapest toll path from a starting vertex to a target vertex using a given number of discount coupons that reduce toll costs by half.

        Each coupon can be applied to one edge, reducing its weight by half (using integer division). This algorithm aims to minimize the total cost of the path while maximizing the use of available coupons.

        Parameters:
            start_id (str): The unique identifier for the starting vertex.
            target_id (str): The unique identifier for the target vertex.
            coupons (int): The maximum number of coupons that can be used to halve the toll cost of an edge.

        Returns:
            Tuple[float, float] | Tuple[None, None]: A tuple containing the minimum total cost of the path and the number of coupons used. Returns (None, None) if no path exists.
        """
        if start_id not in self.vertices or target_id not in self.vertices:
            return None, None 
        pq = TollWayPriorityQueue()
        pq.push(0, 0, self.vertices[start_id])  
        min_costs = {(v.id, coupon): float('inf') for v in self.vertices.values() for coupon in range(coupons + 1)}
        ...
        while not pq.empty():
            ...
            for neighbor_id, weight in current_vertex.adj.items():
                next_vertex = self.vertices[neighbor_id]
                if current_cost + weight < min_costs[(neighbor_id, coupons_used)]:
                    min_costs[(neighbor_id, coupons_used)] = current_cost + weight
                    pq.push(current_cost + weight, coupons_used, next_vertex)
                if coupons_used < coupons:
                    reduced_cost = current_cost + (weight // 2)
                    if reduced_cost < min_costs[(neighbor_id, coupons_used + 1)]:
                        min_costs[(neighbor_id, coupons_used + 1)] = reduced_cost
                        pq.push(reduced_cost, coupons_used + 1, next_vertex)
        return None, None
